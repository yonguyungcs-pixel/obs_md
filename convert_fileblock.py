...
fileblock_path = r"Software\Microsoft\Office\16.0\Word\Security\FileBlock"

try:
    key = winreg.CreateKey(winreg.HKEY_CURRENT_USER, fileblock_path)
    winreg.SetValueEx(key, "Word2003Files", 0, winreg.REG_DWORD, 0)
    winreg.SetValueEx(key, "Word2000Files", 0, winreg.REG_DWORD, 0)
    winreg.SetValueEx(key, "Word97Files", 0, winreg.REG_DWORD, 0)
    winreg.SetValueEx(key, "OpenInProtectedView", 0, winreg.REG_DWORD, 0)
    winreg.CloseKey(key)
    print("Disabled File Block in Registry.")
except Exception as e:
    print(f"Failed to set File Block: {e}")
